...
def gradebook_list_view(request):
    ...
    return render(request, 'reports/gradebook_list.html', {
        'tab': tab,
...
def gradebook_detail_view(request, enrollment_id):
    ...
    if request.method == 'POST':
        ...
        assignment.grading_notes = request.POST.get(
            'grading_notes', ''
        ).strip()
    ...
    return render(request, 'reports/gradebook_detail.html', {
        'enrollment': enrollment,
        'summary': summary,
...
def gradebook_transcript_view(request, child_id):
    ...
    if total_credits > 0:
        overall_gpa = (weighted_gpa_sum / total_credits).quantize(
            Decimal('0.01')
        )
    ...
    if request.GET.get('format') == 'pdf':
        ...
        filename = (
            f"transcript_{child.first_name}_{timezone.now().date()}"
            .replace(' ', '_')
        )
